    ...
    public class FlowCatchmentComponent : GroundHogComponent
    {
        ...
        protected override void RegisterInputParams(GH_InputParamManager pManager)
        {
            ...
            pManager.AddNumberParameter(
                "Proximity Threshold", "T", "The distance between end points required to form a catchment", GH_ParamAccess.item);
        ...
        {
            pManager.AddCurveParameter(
                "Catchments", "B", "The catchment boundaries identified", GH_ParamAccess.tree);
            pManager.AddCurveParameter(
                "Flow Paths", "P", "The flow paths grouped by catchment", GH_ParamAccess.tree);
            pManager.AddColourParameter(
                "Color Codes", "C", "Colour codes the uniquely identify each path and boundary", GH_ParamAccess.tree);
            pManager.AddNumberParameter(
                "Volumes %", "V", "The % of the total flow paths that drain to this area", GH_ParamAccess.tree);
        }
        ...
        protected override void GroundHogSolveInstance(IGH_DataAccess DA)
        {
            ...
            else if (MIN_PROXIMITY == 0)
            {
                ...
                if (FLOW_PATHS[0].TryGetPolyline(out samplePath))
                {
                    ...
                    this.AddRuntimeMessage(GH_RuntimeMessageLevel.Remark, $"Proximity Threshold parameter not provided; guessed {MIN_PROXIMITY} as a good value");
                }
            }
            ...
            this.CreateVoronoi(flowStructures);
            ...
            foreach (var originFlowStructure in flowStructures)
            {
                ...
                foreach (var searchFlowStructure in flowStructures)
                    if (originFlowStructure != searchFlowStructure)
                    {
                        ...
                        if (distance <= searchFlowStructure.groupDistance + this.docUnitTolerance)
                        {
            ...
            for (var v = 0; v < flowStructures.Count; v++)
                if (holdingListBounds[v] != null) // Check the group exists
                {
                    ...
                    var culledEdges = this.DeduplicateLines(groupEdges);
            ...
            for (var i = 0; i < holdingListCurves.Length; i++)
                if (holdingListCurves[i] != null)
                {
                    ...
                    groupedColors.AddRange(
                        this.GenerateGroupColors(shuffledIndices.ElementAt(i), holdingListCurves[i].Count,
        ...
        private List<Line> DeduplicateLines(List<Line> inputLines)
        {
            ...
            for (var j = i - 1; j >= 0; j--)
            {
                ...
                if ((d1 < this.docUnitTolerance || d2 < this.docUnitTolerance) &&
                    (d3 < this.docUnitTolerance || d4 < this.docUnitTolerance))
                {
