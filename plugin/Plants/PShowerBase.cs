namespace Groundhog
{
    ...
    public abstract class PShowerBase : GroundHogComponent
    {
        ...
        protected bool SetupSharedVariables(IGH_DataAccess da)
        {
            ...
            this.PLANT_SPECIES = new List<PlantSpecies>();
            ...
            if (this.PLANT_TIME < 0)
            {
                this.AddRuntimeMessage(
                    GH_RuntimeMessageLevel.Remark, "The specified time was less than zero so no species have been allocated locations.");
                return false;
            }

            if (this.PLANT_LOCATIONS.Count > this.PLANT_SPECIES.Count)
            {
                this.AddRuntimeMessage(
                    GH_RuntimeMessageLevel.Remark, "There were more locations provided than species, so some locations have not been allocated plants");
                PLANT_LOCATIONS.RemoveRange(PLANT_SPECIES.Count, this.PLANT_LOCATIONS.Count - this.PLANT_SPECIES.Count);
            }

            if (this.PLANT_SPECIES.Count > this.PLANT_LOCATIONS.Count)
            {
                ...
                PLANT_SPECIES.RemoveRange(PLANT_LOCATIONS.Count, this.PLANT_SPECIES.Count - this.PLANT_LOCATIONS.Count);
            }

            return true;
        }

        protected static PlantSpecies GetPlantInstance(List<PlantSpecies> plantSpecies, int index, List<GH_String> allLabels, List<Color> allColours)
        {
