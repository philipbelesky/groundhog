...
namespace groundhog
{
    ...
    {
        private static Predicate<double> _isZero = IsZero;
        ...
        {
            pManager.AddPointParameter("Start", "P", "The initial starting point or points for the path(s)",
                GH_ParamAccess.list);
            pManager.AddNumberParameter("Step Size", "L",
                "The distance to move forward for each step. If provided as a list a random option will be selected for each step.",
                GH_ParamAccess.list);
            ...
            pManager.AddIntegerParameter("Random Seed", "S",
                "The random seed to be used for each of the path distance and angle choices. If set the same random results will be produced; if not set they will be different for each run.",
                GH_ParamAccess.item);
            pManager[3].Optional = true;
            pManager.AddNumberParameter("Directions", "D",
                "The possible angles in which to move (as a list of numbers in degrees). If not set a random direction in a 360 degree range will be used.",
                GH_ParamAccess.list);
            pManager[4].Optional = true;
            pManager.AddCurveParameter("Boundary", "B",
                "A boundary (must be a closed planar curve) that the steps will not be allowed to cross",
                GH_ParamAccess.item);
        ...
        protected override void GroundHogSolveInstance(IGH_DataAccess DA)
        {
            ...
            if (negativeIndex != -1)
            {
                AddRuntimeMessage(GH_RuntimeMessageLevel.Error,
                    string.Format("Distances cannot be zero. At least one zero-value found at index {0}.",
                        negativeIndex));
                return;
            }
                ...
                for (var i = 0; i < PATH_ORIGINS.Count; i++)
                    if (BOUNDARY.Contains(PATH_ORIGINS[i], Plane.WorldXY, docUnitTolerance) == PointContainment.Outside)
                    {
                        AddRuntimeMessage(GH_RuntimeMessageLevel.Warning,
                            "If using a boundary curve all points must start within in.");
                        return;
                    }
            }
            ...
            for (var step = 1; step < STEP_COUNT; step++)
            {
                if (attempts >= 1000)
                {
                    AddRuntimeMessage(GH_RuntimeMessageLevel.Remark,
                        "One path was unable to find a movement that could not avoid the boundary so it ended before taking all the specified steps.");
                    break;
                }
                ...
                while (attempts < 1000)
                {
                    ...
                    attempts++;
                    if (BOUNDARY == null || BOUNDARY.Contains(stepPoint, Plane.WorldXY, docUnitTolerance) !=
